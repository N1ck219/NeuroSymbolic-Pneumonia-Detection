...
Raw DICOM images were processed using the \texttt{pydicom} library\cite{pydicom}. The pre-processing pipeline consists of the following steps:
\begin{itemize}
...
\begin{itemize}
 ...
 \begin{minipage}{\linewidth}
    ...
    \captionof{figure}{Horizontal Flip.} 
    \label{fig:hor}
	 \end{minipage}
 \vspace{1em} 
 ...
 \begin{minipage}{\linewidth}
    ...
    \captionof{figure}{Shift-Scale Rotate.}
    \label{fig:shift}
	 \end{minipage}
 \vspace{1em} 
	\item \textbf{Intensity Transformations:} Random Brightness and Contrast adjustments (Fig. \ref{fig:Bright}) 
	
 \noindent
 \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figs/trasformazioni/result_3_RandomBrightnessContrast_00008027_009.png}
    \captionof{figure}{Random Brightness.} 
    \label{fig:Bright}
	 \end{minipage}
	
	
	\item \textbf{Noise Injection:} Gaussian Noise (Fig. \ref{fig:gauss_noise}) and Gaussian Blur (Fig. \ref{fig:gauss_blur}).
	
 \noindent
 \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figs/trasformazioni/result_4_GaussNoise_00001836_050.png}
    \captionof{figure}{Gaussian noise.} 
...
\begin{minipage}{\linewidth}
	\centering
	\includegraphics[width=\linewidth]{figs/trasformazioni/result_5_GaussianBlur_00005139_000}
	\captionof{figure}{Gaussian Blur.}
...
\begin{minipage}{\linewidth}
	...
	\captionof{figure}{Overview of the proposed architecture.}
	\label{fig:schema}
\end{minipage}
\vspace{1em} 
...
\begin{minipage}{\linewidth}
	...
	\captionof{figure}{Example of results.} 
	\label{fig:result}
\end{minipage}
\vspace{1em} 
...
\begin{minipage}{\linewidth}
	\centering
	\includegraphics[width=\linewidth]{figs/loss_curve_final.png} 
...
The models were evaluated on their classification accuracy under these corrupted conditions to measure structural resilience. Detailed confusion matrices for all evaluated models across the entire noise spectrum, along with the exact numerical results, are provided in Appendix \ref{sec:appendix_matrices} (see Table \ref{tab:noise_accuracy_extended}, Figure \ref{fig:noise_comparison_p1} and Figure \ref{fig:noise_comparison_p2}).
...
However, as signal corruption increases, the structural limitations of standard CNNs become clearly visible. The ablation baseline (DenseNet Without CeNN) experiences a catastrophic structural collapse at extreme noise levels (90\%), dropping to an accuracy of 30.6\%. Standard ResNet also degrades significantly, falling to 67.9\%. 
...
Most notably, the proposed CeNN architecture exhibits a highly resilient degradation curve. While starting at a lower baseline accuracy due to the MTL constraint, the network only loses 6.2\% accuracy across the entire noise spectrum (dropping from 79.5\% to 73.4\%). The CeNN layer acts as an exceptional deterministic filter, preventing severe degradation and maintaining stable feature extraction regardless of signal corruption.
...
Beyond standard accuracy, analyzing the degradation through the Matthews Correlation Coefficient (MCC) and AUC-ROC provides a deeper understanding of the models' discriminative collapse (detailed comprehensively in Appendix \ref{sec:appendix_matrices}). At 90\% noise, the standard ResNet50 experiences a severe drop in MCC, falling from 0.5727 to a critical 0.3347, indicating a near-random classification capability on minority classes. The Ablation baseline completely collapses (MCC dropping to 0.1481). In stark contrast, the Proposed CeNN architecture exhibits exceptional stability: its MCC only degrades from 0.4995 to 0.4209 under maximum signal corruption. The CeNN layer acts as an exceptional deterministic filter, preventing severe degradation and maintaining stable feature extraction regardless of signal corruption.
...
\begin{table*}[htbp]
	\centering
	\caption{Hardware complexity, inference times, and extended performance metrics (AUC-ROC, MCC) evaluated at a resolution of 512x512 with a batch size of 4.}
	\label{tab:hardware_metrics}
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lcccccccc}
			\toprule
			\textbf{Model} & \textbf{Accuracy} & \textbf{AUC-ROC} & \textbf{MCC} & \textbf{Parameters (M)} & \textbf{FLOPs (G)} & \textbf{Time / Batch (ms)} & \textbf{Throughput (img/s)} & \textbf{Peak VRAM (MB)} \\
			\midrule
			ResNet50 (Baseline) & 0.8582 & 0.9031 & 0.5699 & 23.50 & 42.35 & 70.99 & 56.35 & 310.02 \\
			EfficientNet-B0 (Baseline) & 0.8698 & 0.9283 & 0.6087 & 4.01 & 3.94 & 29.51 & 135.53 & 357.55 \\
			SE-ResNet50 (Baseline) & 0.8583 & 0.9127 & 0.6041 & 26.03 & 42.35 & 79.37 & 50.39 & 426.86 \\
			\midrule
			Ablation: No CeNN & 0.7826 & 0.8626 & 0.4916 & 18.48 & 33.32 & 79.11 & 50.56 & 450.45 \\
			\textbf{Proposed Hybrid CeNN} & \textbf{0.7926} & \textbf{0.8622} & \textbf{0.4995} & \textbf{18.49} & \textbf{34.16} & \textbf{94.90} & \textbf{42.15} & \textbf{534.92} \\
			\bottomrule
		\end{tabular}%
	}
\end{table*}

As detailed in Table \ref{tab:hardware_metrics}, the hardware profiling is now contextualized alongside robust discriminative metrics such as the Area Under the ROC Curve (AUC-ROC) and the Matthews Correlation Coefficient (MCC). While standard models like EfficientNet-B0 achieve peak clean-data performance (AUC: 0.9283) with minimal computational footprint (3.94 GFLOPs), they lack the intrinsic structural robustness provided by the CeNN framework. The integration of the CeNN frontend introduces a negligible structural overhead. Comparing the Ablation model to the Proposed Hybrid CeNN, the parameter count increases by merely 0.01 million (from 18.48 M to 18.49 M). Although the continuous-time dynamics of the CeNN slightly elevate the FLOPs (34.16 G) and peak VRAM usage (534.92 MB), the resulting inference throughput of $\sim$42 images per second remains highly suitable for real-time clinical screening. The baseline performance (MCC of 0.4995) proves that the model maintains strong discriminative capabilities despite the heavier optimization burden of the multi-task formulation.
...
\begin{strip}
	This appendix provides the comprehensive evaluation metrics, including AUC-ROC and MCC, and the detailed confusion matrices alongside their respective degradation trend lines for all the architectures tested during the noise robustness benchmarking.
	\vspace{1em}
%	\centering
	\captionof{table}{Comprehensive Noise Robustness Metrics. Full evaluation including Precision, Recall, F1-Score, Specificity, AUC-ROC, and MCC across key Gaussian noise levels.}
	\label{tab:noise_accuracy_extended}
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{llcccccccc}
			\toprule
			\textbf{Model} & \textbf{Noise} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-Score} & \textbf{Specificity} & \textbf{AUC-ROC} & \textbf{MCC} \\
			\midrule
			\multirow{4}{*}{\textbf{ResNet50}} 
			& 0\%  & 0.8598 & 0.7755 & 0.5509 & 0.6441 & 0.9523 & 0.9075 & 0.5727 \\
			& 30\% & 0.8304 & 0.6938 & 0.4719 & 0.5617 & 0.9377 & 0.8605 & 0.4745 \\
			& 60\% & 0.7930 & 0.5474 & 0.5826 & 0.5644 & 0.8559 & 0.8246 & 0.4292 \\
			& 90\% & 0.6794 & 0.3939 & 0.7282 & 0.5113 & 0.6648 & 0.7428 & 0.3347 \\
			\midrule
			\multirow{4}{*}{\textbf{EfficientNet-B0}} 
			& 0\%  & 0.8722 & 0.7794 & 0.6208 & 0.6911 & 0.9474 & 0.9303 & 0.6182 \\
			& 30\% & 0.8278 & 0.7618 & 0.3670 & 0.4953 & 0.9657 & 0.8766 & 0.4460 \\
			& 60\% & 0.8295 & 0.7096 & 0.4394 & 0.5427 & 0.9462 & 0.8550 & 0.4643 \\
			& 90\% & 0.8134 & 0.6080 & 0.5338 & 0.5685 & 0.8970 & 0.8289 & 0.4516 \\
			\midrule
			\multirow{4}{*}{\textbf{SE-ResNet50}} 
			& 0\%  & 0.8559 & 0.6837 & 0.6965 & 0.6900 & 0.9036 & 0.9135 & 0.5962 \\
			& 30\% & 0.8272 & 0.6313 & 0.6005 & 0.6155 & 0.8951 & 0.8677 & 0.5045 \\
			& 60\% & 0.8246 & 0.6726 & 0.4646 & 0.5496 & 0.9323 & 0.8425 & 0.4569 \\
			& 90\% & 0.8153 & 0.6494 & 0.4296 & 0.5171 & 0.9306 & 0.8177 & 0.4221 \\
			\midrule
			\multirow{4}{*}{\textbf{Ablation (No CeNN)}} 
			& 0\%  & 0.7871 & 0.5260 & 0.7648 & 0.6233 & 0.7938 & 0.8636 & 0.4984 \\
			& 30\% & 0.7755 & 0.5090 & 0.7152 & 0.5947 & 0.7936 & 0.8391 & 0.4579 \\
			& 60\% & 0.7246 & 0.4450 & 0.7933 & 0.5702 & 0.7040 & 0.8181 & 0.4256 \\
			& 90\% & 0.3062 & 0.2485 & 0.9943 & 0.3976 & 0.1003 & 0.7792 & 0.1481 \\
			\midrule
			\multirow{4}{*}{\textbf{Proposed Hybrid CeNN}} 
			& 0\%  & 0.7950 & 0.5408 & 0.7282 & 0.6207 & 0.8150 & 0.8618 & 0.4945 \\
			& 30\% & 0.7701 & 0.5005 & 0.7502 & 0.6005 & 0.7760 & 0.8418 & 0.4660 \\
			& 60\% & 0.7604 & 0.4865 & 0.7331 & 0.5849 & 0.7685 & 0.8269 & 0.4437 \\
			& 90\% & 0.7343 & 0.4541 & 0.7600 & 0.5685 & 0.7266 & 0.8143 & 0.4209 \\
			\bottomrule
		\end{tabular}%
	}
\end{strip}
...
% --- BLOCCO 1: ResNet e SE-ResNet ---
\begin{figure*}[htbp]
	\centering
	% ResNet
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/ResNet50_CM.png}
		\caption{ResNet50 - Confusion Matrices}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/ResNet50_Degradation.png}
		\caption{ResNet50 - Metric Degradation}
	\end{subfigure}
	\vspace{1.5em}
	...
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/SE-ResNet50_CM.png}
		\caption{SE-ResNet50 - Confusion Matrices}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/SE-ResNet50_Degradation.png}
		\caption{SE-ResNet50 - Metric Degradation}
	\end{subfigure}	
%\end{figure*}
\vspace{1.5em}
%\begin{figure*}[htbp]
	...
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/EfficientNet-B0_CM.png}
		\caption{EfficientNet-B0 - Confusion Matrices}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/EfficientNet-B0_Degradation.png}
		\caption{EfficientNet-B0 - Metric Degradation}
	\end{subfigure}
	\caption{Detailed performance degradation and confusion matrices (Part 1).}
...
\begin{figure*}[htbp]
	...
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/Ablation_Model_(No_CeNN)_CM.png}
		\caption{Ablation (No CeNN) - Confusion Matrices}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/Ablation_Model_(No_CeNN)_Degradation.png}
		\caption{Ablation (No CeNN) - Metric Degradation}
	\end{subfigure}
	\vspace{1.5em}	
%\end{figure*}
...
%\begin{figure*}[htbp]
	...
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/Proposed_Hybrid_CeNN_CM.png}
		\caption{Proposed Hybrid CeNN - Confusion Matrices}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{figs/conf_matr_noise/Proposed_Hybrid_CeNN_Degradation.png}
		\caption{Proposed Hybrid CeNN - Metric Degradation}
	\end{subfigure}
%	\caption{Detailed performance degradation and confusion matrices (Part 2).}
	\caption{Detailed performance degradation and confusion matrices (Part 2). The degradation charts highlight the stabilization of MCC and AUC metrics provided by the CeNN frontend compared to standard architectures.}
