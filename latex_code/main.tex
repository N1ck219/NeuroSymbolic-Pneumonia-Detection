...
% Tieni solo un documentclass
\documentclass[a4paper,fleqn]{cas-dc}

% Tieni solo un pacchetto natbib (quello numerico è il più usato per le scienze esatte)
...
% Definiamo alcuni colori eleganti
\definecolor{codegreen}{rgb}{0,0.6,0}
\definecolor{codegray}{rgb}{0.5,0.5,0.5}
\definecolor{backcolour}{rgb}{0.97,0.97,0.97} % Grigio chiarissimo per lo sfondo

% Creiamo il nostro stile personalizzato
\lstdefinestyle{mystyle}{
	backgroundcolor=\color{backcolour},   
	commentstyle=\color{codegreen}\itshape, % Commenti verdi e in corsivo
	keywordstyle=\color{blue}\bfseries,     % Parole chiave blu e in grassetto
	numberstyle=\tiny\color{codegray},      % Numeri di riga piccolini e grigi
	basicstyle=\ttfamily\footnotesize,      % Font "macchina da scrivere" piccolo
	breakatwhitespace=false,         
	breaklines=true,                 % A capo automatico di emergenza
	captionpos=b,                    % Didascalia in basso (se la usi)
	keepspaces=true,                 % Mantiene le indentazioni
	numbers=none,                    % Metti "left" se vuoi i numeri di riga
	showspaces=false,                
	showstringspaces=false,
	showtabs=false,                  
	tabsize=4,                       % Dimensione del TAB
	frame=single,                    % Bordo sottile attorno al codice
	rulecolor=\color{lightgray}      % Colore del bordo
}

% Diciamo a LaTeX di usare sempre questo stile
\lstset{style=mystyle, 
	aboveskip=10pt,  % Spazio sopra il box
	belowskip=10pt   % Spazio sotto il box (prima di "This rule...")
}
%%%Author definitions
...
% --- Titoli Brevi per l'intestazione delle pagine ---
\shorttitle{A Neuro-Symbolic Architecture for Pneumonia Detection} % Titolo breve aggiornato e accademico
\shortauthors{N. Flego et al.} % Aggiunto "et al." per la presenza di coautori
...
The test set was subjected to progressive levels of Gaussian noise injection (0\%, 15\%, 30\%, 45\%, 60\%, and 90\% variance limits). The models were evaluated on their classification accuracy under these corrupted conditions to measure structural resilience. Detailed confusion matrices for all evaluated models across the entire noise spectrum, along with the exact numerical results, are provided in Appendix \ref{sec:appendix_matrices} (see Table \ref{tab:noise_robustness} and Figure \ref{fig:noise_comparison_p1}).
...
The source code, along with detailed setup instructions, can be accessed on GitHub at: \url{https://github.com/TUO_USERNAME/SmallProject_CeNN_ASP}
...
%\begin{table*}[htbp]
%	\centering
%	\caption{Model Accuracy across increasing levels of Gaussian Noise.}
%	\label{tab:noise_robustness}
%	\resizebox{0.8\textwidth}{!}{%
%		\begin{tabular}{lcccccc}
%			\hline
%			\textbf{Model} & \textbf{0\%} & \textbf{15\%} & \textbf{30\%} & \textbf{45\%} & \textbf{60\%} & \textbf{90\%} \\ \hline
%			ResNet (Standard) & \textbf{86.10\%} & 83.47\% & 83.35\% & 82.35\% & 79.35\% & 67.85\% \\
%			SE-ResNet & 86.02\% & 81.33\% & \textbf{83.12\%} & 82.65\% & 82.47\% & \textbf{81.25\%} \\
%			EfficientNet-B0 & 87.40\% & \textbf{83.58\%} & 82.80\% & \textbf{82.83\%} & \textbf{82.90\%} & 81.17\% \\
%			Without CeNN (Ablation) & 83.30\% & 81.27\% & 81.55\% & 81.83\% & 79.40\% & 46.17\% \\ \hline
%			\textbf{Proposed CeNN Hybrid} & 82.35\% & 80.67\% & 80.67\% & 80.23\% & 79.67\% & 78.50\% \\ \hline
%		\end{tabular}%
%	}
%\end{table*}
